...
control_evap_A<- ggplot(aes(x=Initial_pollen_weight, y=Final_pollen_weight, col=Treatment), data=evap_AC) + geom_point(size=4) + labs(title = "Regression for control pollen (no bees) UMass") + theme_classic(base_size=20)  + theme(legend.text.align = 0) +
...
control_evap_I <- ggplot(aes(x=Initial_pollen_weight, y=Final_pollen_weight, col=Treatment), data=evap_IC) + geom_point(size=4) + labs(title = "Regression for control pollen (no bees) NCSU") + theme_classic(base_size=20) + theme(legend.text.align = 0) +
  labs(x="Initial pollen weight", y = "Final pollen weight") +
  ...
  scale_y_continuous(breaks = seq(0.2, 0.4, by = 0.02))
...
ACount_summary<-ACount_no_deaths %>% filter(!avg_pollen_length==0) %>% group_by(Treatment, avg_pollen_length) %>% summarise(CrithAvg=mean(Count)) %>% cbind(ACount_BW) %>% mutate(STDCrith = CrithAvg/BWCrith)

ICount_BW<- ICount_no_deaths %>% group_by(Treatment) %>% summarise(BWCrith=mean(Count)) %>% filter(Treatment=="buckwheat") 

ICount_summary<-ICount_no_deaths %>% filter(!avg_pollen_length==0) %>% group_by(Treatment, avg_pollen_length) %>% summarise(CrithAvg=mean(Count)) %>% cbind(ICount_BW) %>% mutate(STDCrith = CrithAvg/BWCrith)
...
coxme1 <- coxme(Surv(days_alive,Dead) ~ Treatment  + pollen_consumed + scale(Wing_MM) + Colony + (1|Inoc_Date), data = ACount_cons)
Anova(coxme1)
cox1.lm1<-lsmeans(coxme1, specs="Treatment")
pairs(cox1.lm1) #won't run, will try without bee size
coxme1.2 <- coxme(Surv(days_alive,Dead) ~ Treatment  + pollen_consumed + Colony + (1|Inoc_Date), data = ACount_cons)
Anova(coxme1.2) #pollen consumed not significant, so to not lose precious data point will analyze full dataset
cox1.lm2<-emmeans(coxme1.2, specs="Treatment")
pairs(cox1.lm2) #works!


# what if not excluding bees for which we do not have consumption data
ACount <- ACount %>% mutate(days_alive = Days_til_dead)
coxme2 <- coxme(Surv(days_alive,Dead) ~ Treatment  + Colony + (1|Inoc_Date), data = ACount)
Anova(coxme2)
cox2.lm1<-emmeans(coxme2, specs="Treatment")
pairs(cox2.lm1)
...
coxme2.1 <- coxme(Surv(days_alive,Alive) ~ Treatment  + Colony + (1|Inoc_Date), data = ICount_cons_surv)
Anova(coxme2.1)
...
my_path_y<-expression(paste("log(",italic("Crithidia")," count +1) in 0.02 μL"))
...
boxCountSpikeCombined<-ggplot(SpikesCombinedPlot, aes(x=pollen_form, y=log(count+1), color=species)) +
  geom_boxplot(lwd=2) +
  theme_classic(base_size=35) +
...
scale_x_discrete(labels=c(expression("Metab. + WF pollen"),expression("Whole pollen"),expression("Exine + WF pollen"))) + 
  labs(x="Diet treatment", y=my_path_y, title=my_path_box_title)  +
...
boxACountSpike<-ggplot(SpikesA, aes(x=pollen_form, y=log(count+1), color=species)) +
  geom_boxplot(lwd=2) +
  theme_classic(base_size=35) +
  theme(axis.text.x=element_text(color="black", size=30, angle=45, hjust=1),
        axis.text.y=element_text(color="black", face="italic", size= 40),
  ...
  facet_grid(~species) +
    scale_color_viridis(discrete=TRUE) +
scale_x_discrete(labels=c(expression("Metab. + WF pollen"),expression("Whole pollen"),expression("Exine + WF pollen"))) + 
  labs(x="Diet treatment", y=my_path_y, title=my_path_box_titleA)  +
  theme(legend.position="none")
...
my_path_yI<-expression(paste("log(",italic("Crithidia")," count +1) in 0.02 μL"))
my_path_box_titleI<-expression(paste(italic("Crithidia")," counts at NCSU"))

boxICountSpike<-ggplot(SpikesI, aes(x=pollen_form, y=log(count+1), color=species)) +
  geom_boxplot(lwd=2) +
  theme_classic(base_size=35) +
  theme(axis.text.x=element_text(color="black", size=30, angle=45, hjust=1),
        axis.text.y=element_text(color="black", face="italic", size= 40),
  ...
  facet_grid(~species) +
    scale_color_viridis(discrete=TRUE) +
scale_x_discrete(labels=c(expression("Metab. + WF pollen"),expression("Whole pollen"),expression("Exine + WF pollen"))) + 
  labs(x="Diet treatment", y=my_path_yI, title=my_path_box_titleI)  +
  theme(legend.position="none") 
...
boxACount<-ggplot(ACount_no_deaths3, aes(x=Pollen_type, y=Count, color=Flower_family)) +
  ...
  labs(x="Pollen treatment", y=my_path_box_y, title= my_path_box_title)  +
  scale_color_manual(values=c( "black","#E69F00", "#D55E00")) + 
  guides(color=guide_legend(title="Flower family")) +
  theme(legend.position="none") 
...
ACount_no_deaths4$Flower_family<-factor(ACount_no_deaths4$Flower_family,                            levels=c("Non-Asteraceae","Sunflower","Other Asteraceae"))
...
boxACountFull<-ggplot(ACount_no_deaths4, aes(x=Pollen_type, y=Count, color=Flower_family)) +
  ...
  theme(axis.text.x=element_text(color="black", size=40, angle=45, hjust=1),
        ...
        plot.title = element_text(hjust = 0.5)) +
  scale_x_discrete(labels=c(expression("Red maple"),expression("Buckwheat"),expression("Cocklebur"),expression("Marsh elder"),expression("Sunflower"),expression("Ragweed"))) + 
  labs(x="Pollen treatment", y=my_path_y, title= my_path_box_title)  +
  scale_color_manual(values=c( "black","#E69F00", "#D55E00")) + 
  guides(color=guide_legend(title="Flower family")) +
  theme(legend.position="bottom") 
...
boxICountFull<-ggplot(ICount_no_deaths4, aes(x=Pollen_type, y=Count, color=Flower_family)) +
  ...
  theme(axis.text.x=element_text(color="black", size=40, angle=45, hjust=1),
        ...
        plot.title = element_text(hjust = 0.5)) +
 scale_x_discrete(labels=c(expression("Buckwheat"),expression("Red maple"),expression("Sagebrush"),expression("Dog fennel"),c(expression("Baccharis"),expression("Sunflower")),expression("Dandelion"))) +
 labs(x="Pollen treatment", y=my_path_y, title= my_path_box_title2)  +
  scale_color_manual(values=c( "black","#E69F00", "#D55E00")) + 
  guides(color=guide_legend(title="Flower family")) +
  theme(legend.position="bottom") 
