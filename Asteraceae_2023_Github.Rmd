...
https://github.com/llf44/Asteraceae-pollen
...
CombinedSpines<-rbind(ICount_summary,ACount_summary) %>% 
  mutate(Treatment = recode(Treatment, 
                            ...
                            'sagebrush' = 'Sagebrush',
                            'IVA' = 'Marsh elder',
                            'red maple' = 'Red maple Lab2',
                            'MAP' = 'Red maple Lab1')) %>%
  arrange(CrithAvg) %>% 
  filter(Treatment !="Sunflower Lab1" & Treatment != "Red maple Lab2")
...
# differentiate by color and shape 
library(ggstar)

CombinedSpines <-  CombinedSpines %>%
  arrange(avg_pollen_length) %>% 
  mutate(Treatment=factor(Treatment,levels=Treatment))
  
spinesPlot<-ggplot(CombinedSpines, aes(x = avg_pollen_length, y = STDCrith)) +
  geom_star(aes(starshape= Treatment, fill = Treatment), size = 8) + 
  geom_smooth(method ='lm', linetype="dashed", color="black", size= 3) +
...
#sunflower vs buckwheat = 0.7449379
1-(27.61538/108.26923) 
#sunflower vs red maple = 0.770454
1-(27.61538/120.30435)
#Ragweed (Ambrosia) vs buckwheat = 0.8400452
1-(17.31818/108.26923) 
#Ragweed (Ambrosia) vs red maple = 0.8560469
1-(17.31818/120.30435)
#Cocklebur (Xanthium) vs buckwheat = 0.6178508
1-(41.37500/108.26923)
#Cocklebur (Xanthium) vs red ample = 0.6560806
1-(41.37500/120.30435)
#marsh elder (Iva) vs buckwheat = 0.6104482
...
#sunflower vs buckwheat = 0.7677593
1-(12.676471/54.583333)
#sunflower vs red maple = 0.7602892
1-(12.676471/52.882353)
#Dandelion (Taraxacum) vs buckwheat = 0.9214831
1-(4.285714/54.583333)
#Dandelion (Taraxacum) vs red maple = 0.9189576
1-(4.285714/52.882353)
#Dog fennel (Eupatorium) vs buckwheat = 0.6851145
1-(17.187500/54.583333)
#Dog fennel (Eupatorium) vs red maple = 0.6749861
1-(17.187500/52.882353)
#Baccharis vs buckwheat = 0.5839448
1-(22.709677/54.583333)

#average from ragweed, cocklebur, dandelion, and dog fennel
avg_signif = (0.8400452 + 0.8560469 + 0.6178508 + 0.6560806 + 0.9214831 + 0.9189576 + 0.6851145 + 0.6749861)/8
...
```{r}
table(ICount$Dead) #11 dead/248 bees
